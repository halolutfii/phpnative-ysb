...
$totalQuery = $conn->query("SELECT COUNT(*) AS total FROM mata_pelajaran");
$totalRow = $totalQuery->fetch_assoc();
$totalData = $totalRow['total'];
$totalPages = ceil($totalData / $limit);

$sql = "
    SELECT mata_pelajaran.id_matpel, mata_pelajaran.nama_matpel
    FROM mata_pelajaran 
    LIMIT ? OFFSET ?
";
        ...
        <div id="content">
            <div class="container">
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header bg-primary text-white">Data Mata Pelajaran</div>
                            <div class="card-body">
                                <div class="table-responsive">
                                    <a href="/view/pages/matapelajaran/addmatapelajaran.php" class="btn btn-sm btn-outline-primary">Add</a>
                                    <table class="table">
                                        <thead>
                                            <tr>
                                                <th>No.</th>
                                            ...
                                            if ($result->num_rows > 0) {
                                                ...
                                                while ($row = $result->fetch_assoc()) {
                                                    ...
                                                    echo "<td class='d-flex flex-wrap gap-1'>
                                                        <a href='/view/pages/matapelajaran/detailmatapelajaran.php?id=" . $row['id_matpel'] . "' class='btn btn-sm w-auto m-1 btn-outline-info'>Detail</a>
                                                        <a href='/view/pages/matapelajaran/updatematapelajaran.php?id=" . $row['id_matpel'] . "' class='btn btn-sm w-auto m-1 btn-outline-warning'>Update</a>

                                                        <form action='/view/pages/matapelajaran/deletematapelajaran.php' method='POST' onsubmit='return confirm(\"Apakah kamu yakin ingin menghapus mata pelajaran ini?\")' '>
                                                            <input type='hidden' name='id_matpel' value='" . $row['id_matpel'] . "'>
